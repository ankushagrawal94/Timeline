        ...
        if (ts.eventDict[key]){
            ...
            if([self.type  isEqual: @"circle"]){
                ...
                if ([[ts.eventDict[key] objectForKey:@"stage"]  integerValue] == 1) {
                    event = [[EventViewCircle alloc] initWithFrame:CGRectMake(0, height, self.view.frame.size.width/2 + 19, self.view.frame.size.height)];
                }
                else {
                    event = [[EventViewCircle alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2 - 19, height, self.view.frame.size.width/2 + 19, self.view.frame.size.height)];
                }
